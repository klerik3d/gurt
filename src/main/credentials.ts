...
async function read(): Promise<CredentialsFile> {
  try {
    ...
    const credentials = rawCredentials
      .map((e: unknown) => unseal(e, electron))
      .filter((e: CredentialEntry | null): e is CredentialEntry => e !== null)
...
async function rawSealedByEntry(): Promise<Map<string, Record<string, string>>> {
  ...
  try {
    const raw = JSON.parse(await fs.readFile(credentialsFile(), 'utf8'))
    const rawCredentials = Array.isArray(raw?.credentials) ? raw.credentials : []
    for (const e of rawCredentials) {
      const entry = e as { id?: unknown; sealed?: unknown }
      if (entry && typeof entry.id === 'string' && entry.sealed && typeof entry.sealed === 'object')
        map.set(entry.id, entry.sealed as Record<string, string>)
    }
  } catch {
