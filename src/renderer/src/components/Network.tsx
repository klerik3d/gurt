...
/**
 * The session's network control: internal isolation as an internal/open toggle,
 * the built-in denylist, and the one list the user edits.
 *
...
export function NetworkPicker({
  ...
  // it hides: a folded block still says which mode the session is on. One fold,
  // no folds inside it — opening shows everything at once. Opens already open
  // when there is an allow list, so an edited draft doesn't look like it lost
  // its list.
      ...
      <div className="hc-block">
        <div className="seclabel-row">
          <button type="button" className="subfold" onClick={() => setOpen((o) => !o)}>
            ...
            />
            network
            <span className="strong">{internal ? 'internal' : 'open'}</span>
          </button>
          <InfoDot text={`Internal — ${NET_INFO.internal.hint} Open — ${NET_INFO.open.hint}`} />
        </div>
        {open && (
      ...
      {open && (
        <>
          {/* Applies in both toggle states, because both enforce it on proxied
              ...
              session is a separate task (§6.4). */}
          <div className="hc-block">
            <div className="seclabel-row">
