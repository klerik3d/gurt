...
// --- a secret straddling a chunk boundary ---------------------------------
test('a secret split across two chunks is joined before redaction', () => {
  const lines = []
...
test('redaction holds at a different split offset', () => {
  const lines2 = []
...
test('a chunk split mid multi-byte character decodes intact', () => {
  const lines3 = []
