...
3. Provisioning binds that directory **read-only** at `/gurt/skills` in
   ...
   **Where the link lands is a property of the agent kind.** gurt runs
   ...
   | codex | codex-acp@1.10.0 (bundles @openai/codex@0.153.4) | `.agents/skills` — default-on skills subsystem; the canonical directory (the `~/.codex/skills` twin is deprecated), never `~/.claude/skills` |
